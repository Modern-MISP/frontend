...
<div in:fly={{ x: -200 }} out:fly={{ x: -200 }} class="flex items-center gap-2 absolute z-50">
  {#if type === 'object'}
    <IconCardRow class="border-2 border-sky">
      ...
      <IconCard icon="bx:collapse" text="Collapse" class="text-ctp-red!" on:click={collapseNode} />
    </IconCardRow>
  {/if}
  ...
  {#if $mode === 'edit'}
    <IconCardRow class="border-2 border-sky">
      ...
      {/if}
      <IconCard icon="mdi:delete" text="Delete" class="text-ctp-red!" on:click={deleteNode} />
    </IconCardRow>
  {/if}
</div>
